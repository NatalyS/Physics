 ...
 *
 * @author Jonathan Bingham, Nataliya Sorokina <nvsorokina@herzen.spb.ru>
 */
public class Ruler
        extends JLabel {
    ...
    public boolean isAreLabelsVisible() {
        return areLabelsVisible;
    }

    public void setAreLabelsVisible(boolean areLabelsVisible) {
    ...
    public void computeParameters() {
        ...
        if (this.getPosition() == TOP || this.getPosition() == BOTTOM) {
            ...
            //Compute coordinates needed to draw ticks.
            tickBase = size.width - insets.right;
            //2: gap btwn label & tick
            tickEnd = insets.top + fontMetrics.getHeight() + 2;
            minorTickEnd = (int) (tickEnd + (tickBase - tickEnd) * 2 / 3);
            middleTickEnd = (int) (tickEnd + (tickBase - tickEnd) / 3);

            unitsPerPixel = (max - min) / (size.height - insets.top - insets.bottom);
            if (unitsPerPixel <= 0)
                return;
            ...
            if (pixelMark < insets.top)
                pixelMark = insets.top;
            setMark(mark);
        }
    ...
    public void paint(Graphics g) {
        ...
        if (this.getPosition() == TOP || this.getPosition() == BOTTOM) {
            ...
            int numTicks = (int) (
                    (((double) (rect.y - insets.top)
                            / (double) (size.height - insets.top - insets.bottom)) //% not shown on left
                            * (max - min)) //times range = ruler value in units from left end
                            / unitsPerTick); //convert to ticks
            if (numTicks < 0)
                numTicks = 0; //may be at far left, so insets screwed up calculation
            ...
            //Starting point to paint
            double y = size.height - insets.bottom - numTicks * pixelsPerTick;

            //May need to start with a fractional tick
            //BUG: only works for units/tick > 1
            double fractionalTicks = (min % unitsPerTick) / unitsPerTick;
            if (fractionalTicks != 0d) {
                y -= (1 - fractionalTicks) * pixelsPerTick;
                numTicks++;
            }
            ...
            int labelBase = fontMetrics.getAscent() + insets.top + this.getWidth();
            int inty = (int) Math.round(y);
            while (inty >= yend) {
                //Draw major tick with label
                if ((numTicks + ticksOff) % minorTicks == 0) {
                    g.drawLine(tickBase, inty, tickEnd, inty);
                    ...
                    if (areLabelsVisible && (unitsPerTick * numTicks) % 1 == 0) {
                        ...
                        newTransform.setToRotation(-Math.PI / 2);
                        g2.setTransform(newTransform);

                        int delta = (int) Math.floor(pixelsPerTick * minorTicks);
                        g2.drawString(tickLabel, -(inty + fontMetrics.stringWidth(tickLabel) / 2 + delta), labelBase * 0.3f);
                        g2.setTransform(orig);
                    }
                }
                //Draw middle tick
                else if ((numTicks + ticksOff) % (minorTicks / 2) == 0) {
                    g.drawLine(tickBase, inty, middleTickEnd, inty);
                }
                //Draw minor tick
                else
                    g.drawLine(tickBase, inty, minorTickEnd, inty);
                y -= pixelsPerTick;
            ...
            if (isSliderVisible) {
                ...
                int mark = (int) Math.round(pixelMark);
                g.setColor(Color.red);
                g.drawLine(0, mark, size.width, mark);
                g.setColor(fg);
            }
        }
    }
